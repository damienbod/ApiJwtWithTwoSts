...
public class Worker : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        static async Task RegisterApplicationsAsync(IServiceProvider provider)
        {
            ...
            // API application CC
            if (await manager.FindByClientIdAsync("CC_STS_B") == null)
            {
                await manager.CreateAsync(new OpenIddictApplicationDescriptor
                {
                    ...
                    Permissions =
                    {
                        ...
                        Permissions.Prefixes.Scope + "scope_b"
                    }
                });
            }
            ...
            if (await manager.FindByClientIdAsync("codeflowpkceclient") is null)
            {
                await manager.CreateAsync(new OpenIddictApplicationDescriptor
                {
                    ...
                    {
                        new Uri("https://localhost:44368/signout-callback-oidc"),
                        new Uri("https://localhost:5001/signout-callback-oidc"),
                        new Uri("https://localhost:5001/signout-callback-oidc-t2")
                    },
                    RedirectUris =
                    {
                        new Uri("https://localhost:44368/signin-oidc"),
        ...
        static async Task RegisterScopesAsync(IServiceProvider provider)
        {
            ...
            if (await manager.FindByNameAsync("scope_b") is null)
            {
                await manager.CreateAsync(new OpenIddictScopeDescriptor
                {
